...
interface Props { rows: PropertyRow[] }

export function PropertiesTable({ rows: initialRows }: Props) {
  ...
  const onDragEnd = (event: DragEndEvent) => {
    ...
    startTransition(async () => {
      const res = await reorderProperties(next.map((r) => r.id));
  ...
  const onPublish = () => startTransition(async () => {
    const res = await bulkSetPropertiesPublished(ids(), true);
    if (!res.ok) toast.error(res.error);
    else { toast.success(`Published ${selected.size}`); setSelected(new Set()); }
  });
  const onUnpublish = () => startTransition(async () => {
    const res = await bulkSetPropertiesPublished(ids(), false);
    if (!res.ok) toast.error(res.error);
    else { toast.success(`Unpublished ${selected.size}`); setSelected(new Set()); }
  });
  const onDelete = () => {
    if (!confirm(`Delete ${selected.size} property(ies)? They will be soft-deleted.`)) return;
    startTransition(async () => {
      const res = await bulkDeleteProperties(ids());
      if (!res.ok) toast.error(res.error);
...
function SortableRow({
...
}: { row: PropertyRow; selected: boolean; onToggle: () => void; disabled: boolean }) {
  ...
  const avail = row.unitsTotal != null
    ? `${Math.max(0, row.unitsTotal - row.unitsSold)} / ${row.unitsTotal}`
    : '—';
  ...
  return (
    <TableRow ref={setNodeRef} style={style} data-state={selected ? 'selected' : undefined}>
      ...
      <TableCell>{row.topology && <Badge variant="outline">{row.topology}</Badge>}</TableCell>
      <TableCell className="text-sm">{row.location ?? '—'}</TableCell>
      <TableCell className="text-sm tabular-nums">{row.priceFrom ?? '—'}</TableCell>
      <TableCell className="text-sm tabular-nums">{avail}</TableCell>
      <TableCell><Badge variant={STATUS_VARIANT[row.status]}>{STATUS_LABEL[row.status]}</Badge></TableCell>
      <TableCell>
        {row.publishedAt ? <Badge>Live</Badge> : <Badge variant="outline">Draft</Badge>}
      </TableCell>
      <TableCell className="text-right">
        <Button variant="ghost" size="sm" asChild>
          <Link href={`/admin/properties/${row.id}`}>Edit</Link>
        </Button>
      </TableCell>
    </TableRow>
  );
